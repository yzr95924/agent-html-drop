 ...
 * the text + a collapsible list of quote / comment / author — but NO
 * create / edit / delete: writes need the annotation session cookie, which
 * only the management page's token flow can mint. So this script never
 * sends a credential and never calls a write route.
 *
...
(function () {
  ...
  var collapsed = lsBool(LS_COLLAPSED, false);
  ...
  style.textContent = [
    ...
    "#ahda-panel li.ahda-flash{background:rgba(255,196,0,.18);}",
  ...
  // --- panel skeleton (static template only; annotation data is set via
  //     textContent, never innerHTML — defense against stored XSS) ---
  ...
  // mark click → flash its panel item
  function wireMarks() {
    var marks = document.querySelectorAll("mark[data-anno-id]");
    for (var i = 0; i < marks.length; i++) {
      marks[i].onclick = function (ev) {
        flashItem(ev.currentTarget.getAttribute("data-anno-id"));
      };
    }
  }

  function flashItem(id) {
    var li = $list.querySelector('li[data-anno-id="' + cssEscape(id) + '"]');
    if (!li) return;
    if (!collapsed && li.scrollIntoView) li.scrollIntoView({ block: "nearest", behavior: "smooth" });
    li.classList.add("ahda-flash");
    setTimeout(function () { li.classList.remove("ahda-flash"); }, 1200);
  }

  // panel item click → scroll to its first mark + briefly emphasize it
  ...
  function init() {
    ...
    fetch("/api/files/" + encodeURIComponent(fileName) + "/annotations", { credentials: "omit" })
      ...
      .then(function (data) {
        ...
        if (!entries.length) { panel.remove(); chip.remove(); return; }
        render();
        highlightAll();
      ...
      .catch(function () { panel.remove(); chip.remove(); });
  }
